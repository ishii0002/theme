...
if (isset($_GET['workshop_search']) && !empty($_GET['workshop_search'])) {
    ...
    // Custom search logic
    add_filter('posts_where', function($where) use ($search_keyword){
        global $wpdb;
        $where .= " AND (" . $wpdb->posts . ".post_title LIKE '%" . $search_keyword . "%'";
        $where .= " OR " . $wpdb->posts . ".post_content LIKE '%" . $search_keyword . "%')";
        return $where;
    });
...
if ($workshops_query->have_posts()) {
    ...
    while($workshops_query->have_posts()) {
        ...
        echo '<li class="list">ID: ' . $post_id . ', ワークショップ名: ' . get_the_title() . ', 内容: ' . $content . ', 場所: ' . $place . ', 営業担当者: ' . $salesperson . ', 教師: ' . $teacher . '</li>';
    }
    echo '</ul>';
} else {
    echo '登録されているワークショップはありません。';
