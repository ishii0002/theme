...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_wp_error($user_id)) {
        ...
        $workshop_salesperson = sanitize_text_field($_POST['salesperson']);
        $workshop_teacher = sanitize_text_field($_POST['teacher']);
        ...
        add_post_meta($post_id, 'salesperson', $workshop_salesperson, true); // 追加
        add_post_meta($post_id, 'teacher', $workshop_teacher, true); // 追加
        ...
        wp_redirect('https://ishii.check-demo2.site/page-postformcompletion/?title=' . urlencode($workshop_title) . '&content=' . urlencode($workshop_content) . '&place=' . urlencode($workshop_place) . '&salesperson=' . urlencode($workshop_salesperson) . '&teacher=' . urlencode($workshop_teacher));
...
<form method="POST" action="">
    ...
    <label for="salesperson">定員：</label>
